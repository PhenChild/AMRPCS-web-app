...
export class FormUsuarioComponent implements OnInit {
  ...
  getData(): void {
    ...
    this.dbService
      .getEstacionesParaUsuario(this.filtro)
      .subscribe((data: any) => {
  ...
  selectEstacion(estacion: Estacion): void {
    if (
      this.selectedEstaciones.filter((obj) => obj.id == estacion.id).length <= 0
    ) {
      ...
      if (this.confpassword == this.usuario.password) {
        if (
          ...
          if (confirm('¿Está seguro de crear un nuevo usuario?')) {
            this.dbService
              .addUsuario(this.usuario, this.selectedEstaciones)
              .subscribe(
                (data: any) => {
                ...
                (err: any) => {
                  if (err.status == 418) {
                    this.tService.error(
                      '',
                      'El correo se encuentra en uso por otro usuario.'
                    );
                  } else {
