...
export class PaisesComponent implements OnInit, OnDestroy {
  ...
  getData(): void {
    ...
    this.dbService.getFiltroPaises(this.filtro).subscribe((data: any) => {
  ...
  /**
   * Eliminar una pais
   * @param pais pais que será eliminada
   */
  deletePais(pais: any): void {
    if (confirm('¿Está seguro de eliminar este país?')) {
      this.pais = pais;
      this.dbService.deletePais(this.pais).subscribe(
        (data: any) => {
  ...
  /**
   * Envío de actualización de pais
   * @param pais formulario de pais
   */
  submit(formPais: NgForm): void {
    if (formPais.valid) {
      if (this.isUpdating) {
        if (
          confirm('¿Está seguro de actualizar la información de este país?')
        ) {
          this.dbService.updatePais(this.pais).subscribe(
            (data: any) => {
              this.tService.success(
                'Pais actualizado con éxito.',
                'Envío exitoso'
              );
              formPais.reset();
              const table = <HTMLInputElement>document.getElementById('table');
              const form = <HTMLInputElement>(
                document.getElementById('form-pais')
              );
              table.style.display = 'block';
        ...
        if (confirm('¿Está seguro de crear un nuevo país?')) {
          this.dbService.addPais(this.pais).subscribe(
            (data: any) => {
  ...
  activar(pais: any) {
    if (confirm('¿Está seguro de activar este país?')) {
      this.dbService.activatePais(pais).subscribe(
        (data: any) => {
