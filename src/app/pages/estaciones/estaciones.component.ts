...
export class EstacionesComponent implements OnInit, OnDestroy {
  ...
  getData(): void {
    ...
    this.dbService.getEstaciones(this.filtro).subscribe((data: Estacion[]) => {
  ...
  editarEstacion(estacion: any): void {
    this.dbService.getObservadores(estacion).subscribe((data: any) => {
      this.usuarios = data as any;
      this.dbService.getFotoEstacion(this.estacion).subscribe((data: any) => {
        if (data.foto != undefined) {
          this.estacion.foto = data.foto;
        }
      });
    });
    this.dbService.getTipoRegistros(estacion).subscribe((data: any) => {
      this.variables = data;
  ...
  deleteEstacion(estacion: any): void {
    ...
    if (confirm('¿Está seguro de eliminar esta estación?')) {
      this.dbService.deleteEstacion(this.estacion).subscribe(
        (data: any) => {
  ...
  submit(formEstacion: NgForm): void {
    if (formEstacion.valid) {
      if (
        this.divisiones3.filter((d) => d.id.toString() == this.division3)
          .length == 0
      ) {
        this.tService.error(
          '',
          'Por favor, seleccionar 3 niveles de divisiones políticas.'
        );
      } else {
        if (
          confirm('¿Está seguro de actualizar la información de esta estación?')
        ) {
          this.estacion.idUbicacion = this.division3;
          this.dbService.updateEstacion(this.estacion).subscribe(
            (data: any) => {
              this.tService.success(
                'Estación actualizada con éxito.',
                'Envío exitoso'
              );
              formEstacion.reset();
              const table = <HTMLInputElement>document.getElementById('table');
              const form = <HTMLInputElement>(
                document.getElementById('form-estacion')
              );
              table.style.display = 'block';
              form.style.display = 'none';
              this.getData();
  ...
  editarFoto(estacion: Estacion): void {
    this.estacion = estacion;
    this.dbService.getFotoEstacion(this.estacion).subscribe(
      (data: any) => {
        this.estacion.foto = data.foto;
        const table = <HTMLInputElement>document.getElementById('table');
        const form = <HTMLInputElement>(
  ...
  getDivisiones(pais: any) {
    ...
    this.divisiones2 = [];
    this.divisiones3 = [];
    this.dbService.getDivisionesPaises(pais).subscribe((data: any) => {
  ...
  getDivDivisiones(division: any, nivel: number) {
    if (nivel == 1) {
      ...
      this.divisiones3 = [];
      this.dbService
        .getDivDivisiones(division, nivel)
        .subscribe((data: any) => {
  ...
  verGraficos() {
    ...
    var strDN = dn.getDate() + '/' + dn.getMonth() + '/' + dn.getFullYear();
    var strDF = df.getDate() + '/' + df.getMonth() + '/' + df.getFullYear();
  ...
  activar(estacion: any) {
    if (confirm('¿Está seguro de activar esta estación?')) {
      this.dbService.activateEstacion(estacion).subscribe(
        (data: any) => {
          this.tService.success(
            'Estación activada con éxito.',
            'Envío exitoso'
          );
          this.getData();
        },
        (err: any) => {
          if (err.status == 418) {
            this.tService.error(
              'La estación pertenece a una ubicación inactiva.',
              'Error'
            );
          } else {
            this.tService.error('', 'Ha ocurrido un error');
